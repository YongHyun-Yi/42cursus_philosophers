...
/*
	...
	Think 안에서 반복문이 갇혀있지 않게 하는게 중요함
	왜 바깓의 while 문이 아니라 Think 안에서 대기를 하는지 ? -> 포크대기 중 포크를 어디까지 쥐었는지 보장이 되지 않기 때문
	철학자 구조체에 쥐고있는 포크의 갯수를 넣어놓고 해당 갯수마다 다른 if 문에 걸리도록? -> else if 말고 if 로 처리하면
	앞에서 통과하면 뒤의 조건도 빠르게 통과 할 수 있다
	어떤 state든지 이벤트는 한번만 들러야함

	상태가 변화할때만 출력을 해야함
	상태가 변화했다는 플래그 변수가 있으면 좋을것같음 -> 출력의 조건에 (아무 스레드도 끝나지 않음 && 상태가 변화했음) 을 포함...?
	포크를 어디까지 쥐었는지도 state 화 시켜버리면 하나의 변수에서 관리하니 편할것같기도함
	근데 이렇게하면 Think 에서 포크를 연속해서 집을떄 문제가 됨, 두개를 한번에 집었지만 출력은 take fork 가 안나오고 is eating 만 나옮
	출력할 메세지의 플래그를 배열로 만들면...? -> 진짜 조악하게 구연하는것같다;

	출력 시간이 섞여나오는 문제...

	while 문에 들어가기전에 초반에 1회성 이벤트가 존재
	- 홀수는 철학자 수에 따라 비례적으로 usleep 을 걸어준다
	- 자기 자신의 state 를 출력한다 (초반 공백을 메우기 위해...)
	-- 어차피 usleep 을 쥐고 포크를 못잡으면 state 출력에서 think 가 나오지 않을까...?

	1. 종료조건 체크 (누군가 죽었는지, 내가 죽었는지, 다 먹었는지) -> 먹는횟수 0은 그냥 바로 끝내버리도록 한다
	2. 이벤트 처리
	3. 출력

	과 같은 순서로 코드가 흐르도록 해야함
	이벤트 처리 안에서 반복문이 돌게 만들면
	반복대기하는동안 다른 스레드가가 죽거나 해당 스레드가 죽을수있음
	1번을 통과 즉, 종료조건을 통과한 스레드라는 전제하에 이벤트를 처리함
